...
class BehaveCanoObjectTrajDataset(Dataset):
    def __init__(
        self,
        train,
        data_root_folder,
        ...
        self.selected_seq_names = ['boxlarge', 'boxmedium', 'boxtiny', 'chairwood', 'monitor', 'stool', 'tablesmall', \
                                   'toolbox', 'boxlong', 'boxsmall', 'chairblack', 'plasticcontainer', 'suitcase', \
                                    'tablesquare', 'trashbin']
        ...
        self.rest_object_geo_folder = "processed_data/behave_objects_data/selected_rotated_zeroed_obj_files"
        ...
        if self.test_long_seq:
            dest_obj_bps_npy_folder_for_test = os.path.join(self.data_root_folder, \
                            "behave_objects_bps_npy_files_for_test_joints24_long_seq")
        else:
            dest_obj_bps_npy_folder_for_test = os.path.join(self.data_root_folder, \
                            "behave_objects_bps_npy_files_for_test_joints24")
    ...
    def extract_subset_data(self):
        ...
        for k in self.window_data_dict:
            ...
            index_name = int(seq_name.split("_")[2])
    ...
    def generate_data_for_unseen_objects(self):
        unseen_object_geo_folder = os.path.join(self.data_root_folder, "behave_objects_data/selected_rotated_zeroed_obj_files")

        unseen_object_corr_dict = {
            "largetable": ['boxlong', 'tablesquare'], \
            "smalltable": ['tablesmall',], \
            "woodchair": ['chairwood','stool', 'chairblack'], \
            "largebox": ['boxlarge', 'boxmedium'], \
            "smallbox": ['boxtiny','toolbox', 'boxsmall'], \
            "plasticbox": ['plasticcontainer',],
            "trashcan": ['trashbin'],
            "monitor": ['monitor'],
            "suitcase": ['suitcase',],
        }

        obj_height_json_path = os.path.join(self.data_root_folder, "behave_objects_data/selected_object_height.json")
