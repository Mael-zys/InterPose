...
BLENDER_PATH = os.path.expanduser("blender") # Put your blender path here 
BLENDER_UTILS_ROOT_FOLDER = "./manip/vis" # Put the manip/vis folder absolute path here 
...
def images_to_video(img_folder, output_vid_file, fps=30):
    ...
    command = [
        'ffmpeg', '-r', f'{fps}', '-y', '-threads', '16', '-i', f'{img_folder}/%05d.png', 
        '-preset', 'ultrafast', '-profile:v', 'baseline', '-level', '3.0', 
        '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-an', '-v', 'error', output_vid_file,
    ]
...
def run_blender_rendering_and_save2video(obj_folder_path, out_folder_path, out_vid_path, condition_folder=None, \
    ...
    vis_handpose_and_object=False, hand_pose_path=None, mat_color="blue", vis_condition=False, reset_camera=False, fps=30):
    ...
    reset_camera_param = " --reset-camera" # if reset_camera else ""
    ...
    use_ffmpeg = True
    if use_ffmpeg:
        images_to_video(out_folder_path, out_vid_path, fps=fps)
    else:
